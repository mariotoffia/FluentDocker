...
- **Model** `FluentDocker/Model/` — DTOs, `CommandResponse<T>`, enums, value objects. Legacy exceptions exist: builder configs hold service callbacks, compose configs hold service delegates, and some model builders import Common/Extensions. Build orchestration now lives in `FluentDocker/Kernel/` (`BuildScope`, `BuildResults`, `CapabilityChecks`) because it carries live build state, service results, and composition-root concerns; keep new model code dependency-light and do not reintroduce kernel-facing types under `Model`.
