...
- **Model** `FluentDocker/Model/` — DTOs, `CommandResponse<T>`, enums, value objects. Legacy exceptions exist: builder configs hold service callbacks, compose configs hold service delegates, build/driver scopes carry logging abstractions, and some model builders import Common/Extensions. The Build* orchestration types under `Model/Kernel` (`BuildScope`, `BuildResults`) reference the `Kernel` composition root and `IServiceAsync`, and `Common/CapabilityChecks` references `Kernel` — a documented carve-out (they carry live build state and service results), earmarked to move out of `Model` in v4. Do not move them in v3 (public API); keep new model code dependency-light.
