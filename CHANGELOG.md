...
- **`TemplateString(string, bool handleWindowsPathIfNeeded)` constructor removed.** The opt-in flag converted every `/` to `\` outside quoted URLs, corrupting mixed volume specs (`c:/data:/data` → `c:\data:\data`); it had zero callers inside the library. Use `TemplateString(string)`.
...
- **`ISysCtl.GetCapabilitiesAsync(driverId)` / `IsHealthyAsync(driverId)` / `DefaultDriverId`** — capability and health introspection by driver id, plus the kernel's default driver id, without resolving a typed port first.
- **`IDriverRegistry.IsDisposeComplete` / `AbandonedDriverCount`** — read together: a disposal pass can finish (`IsDisposeComplete == true`) while a budget-exhausted driver was still abandoned (`AbandonedDriverCount > 0`).
- **`ErrorCodes.ModelInference.Disposed` (`MIN_008`)** — terminal, non-retriable code for a model runner/connection disposed while a stream was being enumerated.
- **`ModelOperationGate.TrackedGateCount`** — diagnostic count of live per-model gates; confirms idle gates are evicted rather than leaked.
...
- **`MsTestContainerFixtureBase` renamed to `MsTestPerTestContainerFixtureBase`.** The old name is now an `[Obsolete]` alias (will be removed in a future release) — it read as symmetric with the class-scoped `MsTestClassContainerFixtureBase<T>` but had the opposite (per-test-method) container lifetime. Update subclasses to the new name.
...
- **Dockerfile `ADD` with a rooted source no longer silently ships wrong or stale bytes.** If anything already existed at the staging destination, the real source was never copied — a build could ship an unrelated `COPY`'s file under the `ADD` destination, or a stale copy from an earlier build, with a green build. Rooted sources now stage unconditionally, and `AddCommand` normalizes backslash paths like `CopyCommand` already did.
- **Docker API driver `PullAsync` honors an embedded image tag.** `PullAsync(ctx, "nginx:1.25")` previously stripped the `:1.25` and silently pulled `nginx:latest` instead (the CLI driver already pulled the right image); the API driver now splits `repo:tag` the same way and rejects a conflicting explicit tag instead of discarding it.
- **Wait helpers fail fast on a dead container.** `WaitForPortAsync` / `WaitForHttpAsync` / `WaitForLogMessageAsync` used to burn the full timeout and return `false` with no diagnostics when the container had already exited; they now detect `exited`/`dead` state and throw `FluentDockerException` with the exit code and log tail. `IsRetriableWaitException` also switched from a blacklist to a whitelist, so a genuine programming error no longer silently retries for the full timeout.
- **`ComposeService.StartAsync` / `StopAsync` reconcile before firing hooks.** Hooks and `StateChange` now run only after state is confirmed (matching the existing `RestartAsync` fix) — a stack that crashes on boot no longer runs Running-hooks (migrations, seed data) against a dead deployment.
- **`ResourceExtract` no longer silently drops multi-dot embedded resources.** Files like `Dockerfile.template` or `app.properties` used to extract to an empty directory with no error; matching now uses the fully-qualified resource name, and a matched multi-dot resource extracts under its real name instead of being mangled into a subdirectory + fragment.
- **Podman remote-host machine handling.** Connection failures against a remote daemon (`WithHost("ssh://...")`) were misclassified as "local machine not running" and marked transient; auto-start now fails with guidance instead of guessing when no machine is flagged default, and buffered `StatsAsync` gained `--no-reset` (previously relied on an accidental non-TTY buffering quirk to keep its JSON output clean).
- **Docker CLI Model Runner probe timeouts no longer masquerade as caller cancellation.** A wedged `docker model configure --help` probe used to surface as `OperationCanceledException`, indistinguishable from real caller cancellation; it's now a typed failure. Model Runner CLI operations also null-guard their arguments instead of throwing a raw `NullReferenceException`, and the `DriverContext.RequestTimeout` doc now states it bounds buffered CLI commands too, not only the API driver.
- **Kernel registration-rollback dispose is now budget-bounded.** When driver initialization fails (including caller cancellation), rolling back already-created drivers previously awaited their disposal unbounded — a hung `DisposeAsync` could hang `BuildAsync` forever.
- **Kernel `IsDisposeComplete` docs corrected.** `true` means the disposal pass finished, not that nothing leaked — check `AbandonedDriverCount` alongside it.
- **Kernel `TrySysCtl` stale doc claim removed.** It documented an `InterfaceNotSupportedException` from a resolver path that no longer exists; the actual failure surfaces as `DriverException`.
- **Kernel wrong-typed pack/driver resolution now logs a Warning** naming the actual resolved type instead of Debug-only silence followed by a cause-less exception.
- **`ModelOperationGate` now evicts idle per-model gates** (ref-counted) instead of growing for the life of the process — one semaphore per distinct model reference no longer accumulates in long-lived multi-tenant services.
- **Disposing a model runner mid-stream now surfaces the typed `ModelInference.Disposed` error** instead of a raw `ObjectDisposedException` from inside `HttpClient`.
- **A retried `BuildAsync()` now re-owns its own half-created compose project** instead of silently downgrading to "borrowed" and leaking the stack on flaky-CI retry loops.
- **Cross-scope teardown order no longer rides on `Dictionary` enumeration order** — build scopes dispose in explicit reverse creation order.
- **`WithContextSize` is now validated consistently across all model builders** (three of four previously accepted `0`/negative silently and died deep inside `docker model configure`).
- **Remote-daemon loopback-bound ports now resolve against the daemon host** instead of the client's own loopback — no more false-positive readiness probes against the wrong machine.
- **Named-volume cleanup no longer retries the driver with an already-canceled token** — cancellation stops the per-volume loop instead of being swallowed per volume.
- **Dispose-time export "explode" now uses async, cancelable tar extraction** instead of a sync, uncancelable call that defeated the dispose budget.
- **`DockerApiServiceDriver.RemoveAsync` now attempts every id and aggregates failures** instead of aborting on the first one and silently leaving later ids un-removed.
- **Docker API `GetLogsAsync` falls back to a TTY inspect before byte-sniffing on pre-1.42 daemons** — TTY containers emitting header-like binary output are no longer silently shredded.
- **`StreamIdleTimeout` docs now state its full blast radius** — it applies to every streamed response (events/stats/attach, not just logs) and each timeout abandons a pooled buffer.
