...
- **Kernel hardening.** Registration-rollback dispose is now budget-bounded (was an unbounded hang on cancel); `IsDisposeComplete` docs now say to check `AbandonedDriverCount` too; the stale `TrySysCtl`-throws-`InterfaceNotSupportedException` doc claim is gone; wrong-typed pack/driver resolution now logs a Warning naming the actual type instead of Debug-only silence.
- **Docker Model Runner hardening.** `ModelOperationGate` now evicts idle per-model gates (ref-counted) instead of growing for the life of the process; disposing a runner mid-stream now surfaces the typed `ModelInference.Disposed` error instead of a raw `ObjectDisposedException` from inside `HttpClient`.
- **Builders hardening.** A retried `BuildAsync()` now re-owns its own half-created compose project instead of silently downgrading to "borrowed" and leaking the stack; cross-scope teardown order no longer rides on `Dictionary` enumeration order; `WithContextSize` is now validated consistently across all model builders (three of four previously accepted `0`/negative silently).
- **Services hardening.** Remote-daemon loopback-bound ports now resolve against the daemon host instead of the client's own loopback; named-volume cleanup no longer retries the driver with an already-canceled token; dispose-time export "explode" now uses async, cancelable tar extraction.
- **Docker API driver hardening.** `DockerApiServiceDriver.RemoveAsync` now attempts every id and aggregates failures instead of aborting on the first one; `GetLogsAsync` falls back to a TTY inspect before byte-sniffing on pre-1.42 daemons; `StreamIdleTimeout` docs now state it applies to every streamed response (events/stats/attach, not just logs) and its per-timeout buffer-leak trade-off.
