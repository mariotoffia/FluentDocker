...
namespace FluentDocker.Drivers.Docker.Api.Components
{
    ...
    public class DockerApiStreamDriver : DockerApiDriverBase, IStreamDriver
    {
            ...
            [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ...
            if (config.Types?.Count > 0) filters["type"] = config.Types;
            if (config.Actions?.Count > 0) filters["event"] = config.Actions;
            ...
            Stream stream, [EnumeratorCancellation] CancellationToken ct)
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                if (payloadRead < frameSize) yield break;
        ...
        private static ContainerStats ParseContainerStats(string json)
        {
            ...
            // CPU calculation
            var cpuStats = obj["cpu_stats"] as JObject;
            var preCpuStats = obj["precpu_stats"] as JObject;
            ...
            // Memory
            var memStats = obj["memory_stats"] as JObject;
            if (memStats != null)
            {
            ...
            // Network
            var networks = obj["networks"] as JObject;
            if (networks != null)
            {
            ...
            // Block IO
            var blkio = obj["blkio_stats"]?["io_service_bytes_recursive"] as JArray;
            if (blkio != null)
            {
                foreach (var entry in blkio)
                {
                    ...
                    if (op == "read") stats.BlockRead += value;
                    else if (op == "write") stats.BlockWrite += value;
                }
            }
