    ...
    public class DockerApiServiceDriver : DockerApiDriverBase, IServiceDriver
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (filter != null)
            {
                ...
                if (!string.IsNullOrEmpty(filter.Name)) filters["name"] = [filter.Name];
                if (!string.IsNullOrEmpty(filter.Id)) filters["id"] = [filter.Id];
                if (!string.IsNullOrEmpty(filter.Mode)) filters["mode"] = [filter.Mode];
                if (filters.Count > 0)
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (config.Tail.HasValue) path += $"&tail={config.Tail.Value}";
            if (config.Timestamps) path += "&timestamps=true";
            if (!string.IsNullOrEmpty(config.Since)) path += $"&since={config.Since}";
        ...
        private static object BuildContainerSpec(ServiceCreateConfig config)
        {
            ...
            if (config.Command?.Length > 0) containerSpec["Command"] = config.Command;
            if (config.Args?.Length > 0) containerSpec["Args"] = config.Args;
            if (!string.IsNullOrEmpty(config.User)) containerSpec["User"] = config.User;
            if (!string.IsNullOrEmpty(config.WorkDir)) containerSpec["Dir"] = config.WorkDir;
            ...
            if (config.Mounts?.Count > 0)
                ...
                {
                    Type = m.Type ?? "volume", Source = m.Source,
                    Target = m.Target, ReadOnly = m.ReadOnly
                }).ToList();
        ...
        {
            if (json == null) return new ServiceDetails();
