...
namespace FluentDocker.Drivers.Docker.Api.Components
{
    ...
    public partial class DockerApiImageDriver : DockerApiDriverBase, IImageDriver
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (filter != null)
            {
                if (filter.All) query.Add("all=true");
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!string.IsNullOrWhiteSpace(body))
            {
                ...
                foreach (var item in items ?? new JArray())
                {
                    ...
                    if (!string.IsNullOrEmpty(deleted)) removeResult.Deleted.Add(deleted);
                    if (!string.IsNullOrEmpty(untagged)) removeResult.Untagged.Add(untagged);
                }
            }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (all) filters["dangling"] = new List<string> { "false" };
            if (filter != null)
            {
            ...
            if (result.Data?["ImagesDeleted"] is JArray deleted)
            {
                foreach (var item in deleted)
                {
                    ...
                    if (!string.IsNullOrEmpty(d)) pruneResult.ImagesDeleted.Add(d);
                    else if (!string.IsNullOrEmpty(u)) pruneResult.ImagesDeleted.Add(u);
                }
            }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            Stream stream;
            try { stream = await GetRawStreamAsync(path, cancellationToken); }
            catch (Exception ex)
            {
            ...
            CancellationToken cancellationToken = default)
        {
                ...
                path, content, cancellationToken))
            {
                ...
                if (!string.IsNullOrWhiteSpace(status)) importedId = status.Trim();
        ...
        {
            if (token == null) return new Image();
            return new Image
            {
        ...
        {
            if (token == null) return new ImageLayer();
