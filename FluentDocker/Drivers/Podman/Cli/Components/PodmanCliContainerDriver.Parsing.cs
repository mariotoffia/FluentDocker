...
namespace FluentDocker.Drivers.Podman.Cli.Components
{
    public partial class PodmanCliContainerDriver
    {
        ...
        private static IDictionary<string, string> ParseStringDictionary(JToken token)
        {
            if (token is not JObject obj) return null;
            return obj.Properties().ToDictionary(p => p.Name, p => p.Value.Value<string>());
        }

        private static IDictionary<string, object> ParseExposedPorts(JToken token)
        {
            if (token is not JObject obj) return null;
        ...
        private static DateTime ParseDateTime(JToken token)
        {
            ...
            // Newtonsoft auto-parses date strings into DateTime during JObject.Parse(),
            // so handle both pre-parsed DateTime tokens and raw strings.
